...
use time_primitives::{
	MakeTask, Network, PublicKey, ShardId, ShardsInterface, TaskDescriptorParams, TssPublicKey,
};
...
impl MakeTask for MockTasks {
	fn make_task(_: AccountId, _: TaskDescriptorParams) -> sp_runtime::DispatchResult {
		Ok(())
	}
}
...
frame_support::construct_runtime!(
	pub struct Test
	{
		...
		Gmp: pallet_gmp::{Pallet, Call, Storage, Event<T>},
	}
);
...
impl pallet_gmp::Config for Test {
	type RuntimeEvent = RuntimeEvent;
	type WeightInfo = ();
