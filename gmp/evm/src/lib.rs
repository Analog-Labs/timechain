...
impl IConnectorAdmin for Connector {
    ...
    async fn deploy_gateway(
        ...
        let gateway_address = self
            .deploy_gateway_contract(&config, proxy_address, gateway)
            .await?;
    ...
    async fn deploy_test(&self, gateway: Address32, tester: &[u8]) -> Result<(Address32, u64)> {
        let call = sol::GmpTester::constructorCall {
            gateway: a_addr(gateway),
        };
    ...
    async fn set_admin(&self, gateway: Address32, admin: Address32) -> Result<()> {
        let call = sol::Gateway::setAdminCall {
            admin: a_addr(admin),
        };
    ...
    async fn shards(&self, gateway: Address32) -> Result<Vec<TssPublicKey>> {
        let keys = self
            .evm_call(gateway, sol::Gateway::shardsCall {})
            .await?
            ._0;
    ...
    async fn routes(&self, gateway: Address32) -> Result<Vec<Route>> {
        let routes = self
            .evm_call(gateway, sol::Gateway::routesCall {})
            .await?
            ._0;
    ...
    async fn send_message(
        ...
        receipt
            ...
            .filter(|e| {
                e.topics()
                    .contains(&sol::Gateway::GmpCreated::SIGNATURE_HASH)
            })
    ...
    async fn recv_messages(
        ...
        Ok(logs
            .filter(|e| {
                future::ready(
                    e.topics()
                        .contains(&sol::GmpTester::MessageReceived::SIGNATURE_HASH),
                )
            })
    ...
    async fn max_fee_per_gas(&self) -> Result<u128> {
        ...
        let base_fee = block
            .base_fee_per_gas
            .ok_or(anyhow!("Failed to get latest base fee"))?;
        ...
        Ok(fee_estimator
            .estimate(base_fee.into(), &rewards)
            .max_fee_per_gas)
    }
...
impl Connector {
    ...
    async fn deploy_contract_with_factory(
        ...
        let topic = log
            .topics()
            .first()
            .with_context(|| format!("tx {tx_hash} topic not found"))?;
    ...
    async fn deploy_factory_contract(&self, config: &DeploymentConfig) -> Result<()> {
        ...
        self.transfer(deployer_address, config.required_balance)
            .await?;
    ...
    async fn process_cctp_msg(&self, request: &mut CctpRequest) -> Result<(), CctpError> {
        ...
        if cctp_payload
            .get_version()
            .map_err(|_| CctpError::InvalidPayload)?
            != 0
        {
        ...
        let signature = attestation_response
            .attestation
            .clone()
            .ok_or(CctpError::AttestationResponse)?;
