use crate::mock::*;
use crate::{Event, Heartbeat, MemberNetwork, MemberOnline, MemberPeerId, Pallet, TimedOut};
...
fn unregister_member(pubkey: [u8; 32]) -> DispatchResult {
	Members::unregister_member(RawOrigin::Root.into(), pubkey.into())
}
...
#[test]
fn can_register_member_without_balance() {
	new_test_ext().execute_with(|| {
		assert_ok!(register_member(C));
	});
}
