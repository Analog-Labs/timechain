...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		MultisigApproval(T::AccountId, BridgeTimepoint<BlockNumberFor<T>>, T::AccountId, [u8; 32]),
		...
		MultisigCancelled(T::AccountId, BridgeTimepoint<BlockNumberFor<T>>, T::AccountId, [u8; 32]),
	}
		...
		fn build(&self) {
			{
				...
				data.iter().for_each(|(k, v)| {
					...
					>>::insert::<&T::AccountId, &MultisigAccount<T::AccountId>>(k, v);
				});
			}
		}
	}
...
mod weight_of {
	...
	/// - DB Weight: None
	/// - Plus Call Weight
	pub fn as_multi_threshold_1(call_len: usize, call_weight: Weight) -> Weight {
		WEIGHT_PER_MICROS
			.mul(34)
...
impl<T: Config> Pallet<T> {
	...
	/// # <weight>
	pub fn add_signatory(who: T::AccountId, new_member: T::AccountId) -> DispatchResult {
	...
	/// Total complexity - O(M)
	/// # <weight>
	pub fn remove_signatory(who: T::AccountId, signatory: T::AccountId) -> DispatchResult {
		<Accounts<T>>::mutate(&who, |opt| {
			let multisig = opt.as_mut().ok_or(Error::<T>::UnknownMultisigAccount)?;
	...
	fn inner_as_multi_threshold_1(
		...
		result
			.map(|post_dispatch_info| {
				post_dispatch_info
					.actual_weight
					.map(|actual_weight| weight_of::as_multi_threshold_1(call_len, actual_weight))
			})
			.map_err(|err| (err.post_info.actual_weight, err.error))
	}
