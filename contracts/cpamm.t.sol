...
contract CPAMMTest is Test {
    ...
    function test_liquidity() public {
        ...
        uint256 shares0 = amm.addLiquidity(100, 10);
        ...
        uint256 shares1 = amm.addLiquidity(100, 10);
        assertEq(shares1, 31);

        (uint256 amount0_0, uint256 amount1_0) = amm.removeLiquidity(shares1);
        assertEq(amount0_0, 100);
        assertEq(amount1_0, 10);

        vm.startPrank(alice);
        (uint256 amount0_1, uint256 amount1_1) = amm.removeLiquidity(shares0);
    ...
    function test_swap() public {
        uint256 rate = 10;
        uint256 supply0 = 10000000000000000;
        uint256 supply1 = supply0 * rate;
        token0.mint(supply0);
        token1.mint(supply1);
        ...
        uint256 amount = amm.swap(address(token0), 10000);
