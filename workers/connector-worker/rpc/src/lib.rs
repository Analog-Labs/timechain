...
// Provides RPC methods for interacting with Connector Worker.
#[rpc(client, server)]
pub trait ConnectorRpcApi {
...
pub struct ConnectorRpcApiHandler {
	// this wrapping is required by rpc boundaries
	signer: Arc<Mutex<Sender<(u64, Vec<u8>)>>>,
	kv: ConnectorKeyvault,
}

impl ConnectorRpcApiHandler {
	pub fn new(signer: Arc<Mutex<Sender<(u64, Vec<u8>)>>>, kv: ConnectorKeyvault) -> Self {
		Self { signer, kv }
	}
}

#[async_trait]
impl ConnectorRpcApiServer for ConnectorRpcApiHandler {
	async fn submit_for_signing(
		&self,
		group_id: u64,
		message: Vec<u8>,
