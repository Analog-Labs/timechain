...
use polkadot_sdk::*;
use sp_runtime::{traits::IdentityLookup, BuildStorage, DispatchError, ModuleError, Perbill};
...
fn multisig_deposit_is_taken_and_returned() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Multisig::register_multisig_inner(1, vec![1, 2, 3],));
...
fn cancel_multisig_returns_deposit() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Balances::reserved_balance(1), 0);
		assert_ok!(Multisig::cancel_as_multi(RuntimeOrigin::signed(1), multi, now(), hash),);
		assert_eq!(Balances::free_balance(1), 10);
		assert_eq!(Balances::reserved_balance(1), 0);
	});
}
...
fn already_dispatched_checking_works() {
	new_test_ext().execute_with(|| {
		let multi = Multisig::multi_account_id(&1, 1, 0);
		assert_ok!(Multisig::register_multisig_inner(1, vec![1, 2, 3, 4],));
		let call = RuntimeCall::Balances(BalancesCall::transfer_allow_death { dest: 6, value: 15 });
		let call_weight = call.get_dispatch_info().weight;
...
fn already_dispatched_checking_works_for_threshold_1() {
	new_test_ext().execute_with(|| {
		let multi = Multisig::multi_account_id(&1, 1, 0);
		assert_ok!(Multisig::register_multisig_inner(1, vec![1],));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed(1), multi, 5));
		...
		assert_noop!(
			Multisig::as_multi_threshold_1(RuntimeOrigin::signed(1), multi, boxed_call, timepoint),
			DispatchErrorWithPostInfo {
				error: Error::<Test>::AlreadyDispatched.into(),
				post_info: Pays::No.into()
			}
...
fn multisig_2_of_2_works_with_call_storing() {
	new_test_ext().execute_with(|| {
		let multi = Multisig::multi_account_id(&1, 1, 0);
		assert_ok!(Multisig::register_multisig_inner(1, vec![1, 2],));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed(1), multi, 5));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed(2), multi, 5));
...
fn multisig_only_multisig_can_add_or_remove_signatory() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Multisig::add_signatory(multi, 4));
		assert_ok!(Multisig::remove_signatory(multi, 4));
		assert_err!(Multisig::add_signatory(1, 2), Error::<Test>::UnknownMultisigAccount);
		assert_err!(Multisig::remove_signatory(1, 2), Error::<Test>::UnknownMultisigAccount);
	});
}
...
fn multisig_3_of_3_works_with_new_signatory() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed(2), multi, 5));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed(3), multi, 5));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed(4), multi, 5));
...
fn cancel_multisig_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Multisig::cancel_as_multi(RuntimeOrigin::signed(2), multi, now(), hash),
			Error::<Test>::NotOwner,
		);
		assert_ok!(Multisig::cancel_as_multi(RuntimeOrigin::signed(1), multi, now(), hash),);
	});
...
fn too_many_signatories_fails() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Multisig::register_multisig_inner(1, vec![1, 2, 3, 4, 5],),
...
fn executes_call_on_peer_remove() {
	new_test_ext().execute_with(|| {
		...
		assert!(!crate::DispatchedCalls::<Test>::contains_key(hash, timepoint));
		assert_ok!(Multisig::remove_signatory(multi, 3));
		assert!(crate::DispatchedCalls::<Test>::contains_key(hash, timepoint));
