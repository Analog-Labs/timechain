...
fn authority_id_works() {
	...
	let store: ConnectorKeyvault = Some(store).into();
...
#[test]
fn sign_no_keystore() {
	let store: ConnectorKeyvault = None.into();
...
fn verify_works() {
	...
	assert!(ConnectorKeyvault::verify(&alice, &sig, msg));
