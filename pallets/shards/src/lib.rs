...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		///		2. Validate the commitment length against the shard\'s threshold.
		///		3. Validate each commitment element.
		///		4. Verify the proof of knowledge using the member\'s peer ID.
		///		5. Update the member\'s status to `Committed` and store the commitment.
		///		6. If all members have committed, update the shard\'s state to `Committed` and store the group commitment.
		...
		///		3. Update the member\'s status to `Ready`.
		///		4. If all members are ready, update the shard\'s state to `Online` and emit the [`Event::ShardOnline`] event.
	...
	///		2. Check if any shard\'s DKG process has timed out.
	...
	impl<T: Config> Pallet<T> {
		...
		///   # Flow
		///	1. Update the shard\'s state to `Offline`.
		///	2. Remove the shard\'s threshold.
		///	3. Notify the task scheduler and elections module that the shard is offline.
		///	4. Drain the shard\'s members and remove their corresponding entries.
	...
	impl<T: Config> ShardsInterface for Pallet<T> {
		...
		///	5. Returns the retrieved signer\'s public key.
