...
use time_primitives::{OcwPayload, ShardCreated, ShardId, TimeId, TssPublicKey, TIME_KEY_TYPE};
...
const NOT_COLLECTOR: TimeId = TimeId::new([1; 32]);
...
fn test_ocw_submit_tx() {
	...
	ext.execute_with(|| {
		Ocw::shard_created(PAYLOAD.shard_id(), vec![collector.into()]);
...
fn test_submit_public_key() {
	...
	let collector = keystore.sr25519_generate_new(TIME_KEY_TYPE, Some(&PHRASE)).unwrap();
	ext.execute_with(|| {
		Ocw::shard_created(SHARD_ID, vec![collector.into()]);
	...
	ext.execute_with(|| {
		...
		assert_noop!(
			Ocw::submit_tss_public_key(
				RawOrigin::Signed(NOT_COLLECTOR).into(),
				SHARD_ID,
				TSS_PUBLIC_KEY
			),
