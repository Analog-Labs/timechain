...
impl TransactionsDB {
	...
	pub fn store_tx(&self, tx_data: &TxData) -> Result<()> {
	...
	pub fn remove_tx(&self, hash: H256) -> Result<()> {
		let mut composite_key = [0u8; 64];
		composite_key[..32].copy_from_slice(&self.public_key);
	...
	pub fn load_pending_txs(&self) -> Result<VecDeque<TxData>> {
